...
class ExchangeService(ExchangePort):
    def __init__(
        self,
        requests_history_repository: RequestsHistoryRepositoryPort = Provide[
            "requests_history_repository"
        ],
    ...
    def get_exchange(self, currency_to_calculate: CurrencyToCalculateEntity) -> float:
        request_data = dict(
            ...
            value=currency_to_calculate.value,
        )
        try:
            rate_from = (
            ...
            new_request_entity = NewRequestEntity(
                **request_data, exchange_rate=final_value, is_error=False
            )
            self.requests_history_repository.add(new_request_entity)
